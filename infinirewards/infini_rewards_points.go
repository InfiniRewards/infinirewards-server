...
// @return: The transaction hash and an error
func TransferPoints(ctx context.Context, account *account.Account, pointsContract string, from string, to string, amount *big.Int) (string, error) {
	fromFelt, err := utils.HexToFelt(from)
	if err != nil {
		return "", fmt.Errorf("failed to convert 'from' address to felt: %w", err)
	}
	...
	amountFelt := BigInt256ToFelt(amount)
	calldata := []*felt.Felt{fromFelt, toFelt, amountFelt[0], amountFelt[1]}
...
func GetPointsContractDetails(ctx context.Context, pointsContract string) (string, string, string, uint64, uint64, error) {
	...
	symbol, err := utils.ByteArrFeltToString(resp[currentIndex : currentIndex+count])
	if err != nil {
	...
	count = utils.FeltToBigInt(resp[currentIndex]).Int64()
	description, err := utils.ByteArrFeltToString(resp[currentIndex : currentIndex+count])
	if err != nil {
